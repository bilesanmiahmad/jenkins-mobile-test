...
const NationIdentificationFront = () => {
  const router = useRouter();

  const { ...methods } = useForm();

  const mode = "both" as Mode;
  const imageUri = createStore.getState().frontID;
    ...
    try {
      if (pickerType === "camera") {
        ...
        if (result.canceled) {
          ...
          createStore.setState({ frontID: result.assets[0].uri });
        }
      } else {
      ...
      {imageUri ? (
        ...
        <Box>
          ...
          <Box padding="md" justifyContent="center" gap="md">
            <Text variant="header">National ID card photo (Front)</Text>
            <Box
...
export default NationIdentificationFront;
